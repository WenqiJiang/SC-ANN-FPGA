...
void collect_result_after_stage2(
    ...
    dist_cell_ID_t result_buffer[NLIST];
...
#pragma HLS pipeline II=1
    ...
    // output size = query_num * TOPK 64-bit units
    int output_iter = NLIST < query_num * TOPK? NLIST: query_num * TOPK;
    for (int i = 0; i < output_iter; i++) {
        ap_uint<64> reg;
...
void collect_result_after_stage3(
    ...
    dist_cell_ID_t result_buffer[NPROBE];
...
#pragma HLS pipeline II=1
    ...
    // output size = query_num * TOPK 64-bit units
    int output_iter = NPROBE < query_num * TOPK? NPROBE: query_num * TOPK;
    for (int i = 0; i < output_iter; i++) {
        ap_uint<64> reg;
        int cell_ID = result_buffer[i].cell_ID;
        float dist = result_buffer[i].dist;
