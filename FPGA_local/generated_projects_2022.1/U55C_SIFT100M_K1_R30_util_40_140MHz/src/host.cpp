...
//////////////////////////////   TEMPLATE START  //////////////////////////////
        ...
        std::string HBM_embedding16_dir_suffix("HBM_bank_16_raw");
        std::string HBM_embedding16_dir = dir_concat(data_dir_prefix, HBM_embedding16_dir_suffix);
        std::ifstream HBM_embedding16_fstream(
            HBM_embedding16_dir, 
            std::ios::in | std::ios::binary);
    
        std::string HBM_embedding17_dir_suffix("HBM_bank_17_raw");
        std::string HBM_embedding17_dir = dir_concat(data_dir_prefix, HBM_embedding17_dir_suffix);
        std::ifstream HBM_embedding17_fstream(
            HBM_embedding17_dir, 
            std::ios::in | std::ios::binary);
    
        std::string HBM_embedding18_dir_suffix("HBM_bank_18_raw");
        std::string HBM_embedding18_dir = dir_concat(data_dir_prefix, HBM_embedding18_dir_suffix);
        ...
        HBM_embedding16_fstream.seekg(0, HBM_embedding16_fstream.end);
        size_t HBM_embedding16_size =  HBM_embedding16_fstream.tellg();
        if (!HBM_embedding16_size) std::cout << "HBM_embedding16_size is 0!";
        HBM_embedding16_fstream.seekg(0, HBM_embedding16_fstream.beg);
        HBM_embedding17_fstream.seekg(0, HBM_embedding17_fstream.end);
        size_t HBM_embedding17_size =  HBM_embedding17_fstream.tellg();
        if (!HBM_embedding17_size) std::cout << "HBM_embedding17_size is 0!";
        HBM_embedding17_fstream.seekg(0, HBM_embedding17_fstream.beg);
        HBM_embedding18_fstream.seekg(0, HBM_embedding18_fstream.end);
        size_t HBM_embedding18_size =  HBM_embedding18_fstream.tellg();
        if (!HBM_embedding18_size) std::cout << "HBM_embedding18_size is 0!";
        HBM_embedding18_fstream.seekg(0, HBM_embedding18_fstream.beg);
    ...
    size_t HBM_embedding12_len = (int) (HBM_embedding12_size / sizeof(uint32_t));
    size_t HBM_embedding13_len = (int) (HBM_embedding13_size / sizeof(uint32_t));
    size_t HBM_embedding14_len = (int) (HBM_embedding14_size / sizeof(uint32_t));
    size_t HBM_embedding15_len = (int) (HBM_embedding15_size / sizeof(uint32_t));
    size_t HBM_embedding16_len = (int) (HBM_embedding16_size / sizeof(uint32_t));
    size_t HBM_embedding17_len = (int) (HBM_embedding17_size / sizeof(uint32_t));
    size_t HBM_embedding18_len = (int) (HBM_embedding18_size / sizeof(uint32_t));
...
//////////////////////////////   TEMPLATE START  //////////////////////////////
    ...
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM_embedding12(HBM_embedding12_len, 0);
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM_embedding13(HBM_embedding13_len, 0);
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM_embedding14(HBM_embedding14_len, 0);
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM_embedding15(HBM_embedding15_len, 0);
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM_embedding16(HBM_embedding16_len, 0);
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM_embedding17(HBM_embedding17_len, 0);
    std::vector<uint32_t, aligned_allocator<uint32_t>> HBM_embedding18(HBM_embedding18_len, 0);
...
//////////////////////////////   TEMPLATE END  //////////////////////////////
    ...
    HBM_embedding16_fstream.read(HBM_embedding16_char, HBM_embedding16_size);
        if (!HBM_embedding16_fstream) {
            std::cout << "error: only " << HBM_embedding16_fstream.gcount() << " could be read";
            exit(1);
     }
    HBM_embedding17_fstream.read(HBM_embedding17_char, HBM_embedding17_size);
        if (!HBM_embedding17_fstream) {
            std::cout << "error: only " << HBM_embedding17_fstream.gcount() << " could be read";
            exit(1);
     }
    HBM_embedding18_fstream.read(HBM_embedding18_char, HBM_embedding18_size);
        if (!HBM_embedding18_fstream) {
            std::cout << "error: only " << HBM_embedding18_fstream.gcount() << " could be read";
            exit(1);
     }
...
//////////////////////////////   TEMPLATE START  //////////////////////////////
    ...
    OCL_CHECK(err, cl::Buffer buffer_HBM_embedding12(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 
                HBM_embedding12_size, HBM_embedding12.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_HBM_embedding13(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 
                HBM_embedding13_size, HBM_embedding13.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_HBM_embedding14(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 
                HBM_embedding14_size, HBM_embedding14.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_HBM_embedding15(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 
                HBM_embedding15_size, HBM_embedding15.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_HBM_embedding16(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 
                HBM_embedding16_size, HBM_embedding16.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_HBM_embedding17(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 
                HBM_embedding17_size, HBM_embedding17.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_HBM_embedding18(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, 
                HBM_embedding18_size, HBM_embedding18.data(), &err));
...
//////////////////////////////   TEMPLATE START  //////////////////////////////
    ...
    OCL_CHECK(err, err = krnl_vector_add.setArg(arg_counter++, buffer_HBM_embedding12));
    OCL_CHECK(err, err = krnl_vector_add.setArg(arg_counter++, buffer_HBM_embedding13));
    OCL_CHECK(err, err = krnl_vector_add.setArg(arg_counter++, buffer_HBM_embedding14));
    OCL_CHECK(err, err = krnl_vector_add.setArg(arg_counter++, buffer_HBM_embedding15));
    OCL_CHECK(err, err = krnl_vector_add.setArg(arg_counter++, buffer_HBM_embedding16));
    OCL_CHECK(err, err = krnl_vector_add.setArg(arg_counter++, buffer_HBM_embedding17));
    OCL_CHECK(err, err = krnl_vector_add.setArg(arg_counter++, buffer_HBM_embedding18));
...
//////////////////////////////   TEMPLATE START  //////////////////////////////
    ...
    OCL_CHECK(
        ...
        buffer_HBM_embedding16,
        buffer_HBM_embedding17,
        buffer_HBM_embedding18,
