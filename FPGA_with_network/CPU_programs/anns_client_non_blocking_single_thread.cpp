...
void *thread_query(void* vargp) 
{ 
    struct Thread_info* t_info = (struct Thread_info*) vargp;
    printf("Printing from querying thread...\n"); 
    ...
    // Starting send...
    for (int query_id = 0; query_id < send_recv_gap - 1; query_id++) {
    ...
    // Middle send & recv
    for (int middle_query_id = 0; middle_query_id < query_num - send_recv_gap + 1; middle_query_id++) {

        // send
        send_query_id = middle_query_id + send_recv_gap - 1;
        std::cout << "sending thread query id: " << send_query_id << std::endl;

	    int current_query_sent_bytes = 0;

	    clock_gettime(CLOCK_BOOTTIME, &query_start_time_array[send_query_id]);
        ...
        // recv
        receive_query_id = middle_query_id;
	    ...
	    clock_gettime(CLOCK_BOOTTIME, &query_end_time_array[receive_query_id]); 
    }
    ...
    // Tail recv
    for (int query_id = query_num - send_recv_gap + 1; query_id < query_num; query_id++) {
...
{ 
    ...
    pthread_create(&thread_query_inst, NULL, thread_query, (void*) &t_info); 

    pthread_join(thread_query_inst, NULL); 
    printf("After Thread\n"); 
