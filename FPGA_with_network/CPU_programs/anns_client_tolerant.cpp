// There are some bugs in the FPGA network kernel, or the FPGA cluster
//   Sometimes FPGA does not finish sending the last result(s)
//   In this case we terminate the thread that are not finished
...
{ 
	...
	int current_query_sent_bytes = 0;
        ...
        global_RT_ms_buf[start_query_id + query_id] = response_time_ms;
    }
...
{ 
    ...
    // After 10 sec, if there are still RT in the array not be written,
    //   we assume the program runs into a dead end
    //   the RT array will be saved with some RT marked as large floats
    //   User are required to manually kill the program
	timespec dead_end_detect_start, dead_end_detect_end;
	clock_gettime(CLOCK_BOOTTIME, &dead_end_detect_start);

    while (1) {
        clock_gettime(CLOCK_BOOTTIME, &dead_end_detect_end);

        timespec total_time = diff(dead_end_detect_start, dead_end_detect_end);

        float total_time_ms = 
            ((float) total_time.tv_sec) * 1000.0 + 
            ((float) total_time.tv_nsec) / 1000.0 / 1000.0;

        if (total_time_ms > 10 * 1000) {
            break;
        }
    }
