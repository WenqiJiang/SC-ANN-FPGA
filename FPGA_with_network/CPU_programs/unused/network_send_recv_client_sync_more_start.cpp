...
{ 
	...
	clock_gettime(CLOCK_MONOTONIC  , &query_start_time_array[0]);
	// clock_gettime(CLOCK_THREAD_CPUTIME_ID, &start);

    // auto start = std::chrono::high_resolution_clock::now();

    printf("sending dummy byte for 1 connection\n");
    int send_dummy_bytes = send(sock, send_buf, QUERY_SIZE , 0);
    ...
    for (int query_id = 0; query_id < query_num; query_id++) {
	    ...
	    clock_gettime(CLOCK_MONOTONIC  , &query_start_time_array[query_id + 1]);
